RegisterServerEvent('kng:ricompensahumane')
AddEventHandler('kng:ricompensahumane', function(value)
    local xPlayer = ESX.GetPlayerFromId(source)
    local prezzokey = 2000 -- Prezzo dela chiave
 
     xPlayer.addInventoryItem('money', prezzokey)
     TriggerClientEvent('ox_lib:notify', xPlayer.source, {type = 'success', description = 'Hai Completato Il Lavoro Assegnato Per: ' .. prezzokey .. '$'}) 
end)
...
local resourceName = "Zoxe-Humane"

CreateThread(function()
    if GetCurrentResourceName() ~= resourceName then
        resourceName = "Zoxe-Humane (" .. GetCurrentResourceName() .. ")"
    end
end)
